...
import openconc.settings
import openconc.corpora
import openconc.concordance
import openconc.misc
import os
...
class OpenConc(tk.Frame):
    ...
    def draw_main_ui(self):
        ...
        # --- settings --- #
        self.config_frame = confui.ConfigFrame(self)
        # corpus selection
        self.file_frame = fileui.FileFrame(self)
        # --- concordance --- #
        self.concordance_frame = concui.ConcordanceFrame(self)
        misc.pad_children(self, 5, 5)
        # --- add to main book --- #
        self.book.add(self.file_frame, text="Files")
        self.book.add(self.concordance_frame, text="Concordance")
        self.book.add(self.config_frame, text="Settings")
